...
# Usage: ./auto_model.bash users products orders
...
for table in "$@"; do
    ...
    desc_table=$singular"_descriptions"
    desc_singular=$(singularize_last_word "$desc_table")
    desc_class_name=$(to_camel_case "$desc_singular")

    # Generate model
    cmd="php artisan infyom:scaffold $class_name --fromTable --table=$table"
    $cmd
    # Generate model descriptions
    desc_cmd="php artisan infyom:scaffold $desc_class_name --fromTable --table=$desc_table --skip=controllers,views,requests,routes,menu"
    $desc_cmd
done
