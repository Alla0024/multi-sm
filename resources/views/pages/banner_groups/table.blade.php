<div class="card-body p-0">
    ...
    <div class="table-responsive">
        <table class="table" id="banner-groups-table">
            <thead>
            <tr>
...
{{--            @dump($bannerGroups)--}}
            @foreach($bannerGroups as $bannerGroup)
                <tr>
                    <th>
                        <div class="input-block input-toggle flex">
                            <div class="form-check form-switch">
                                <input class="form-check-input checkbox-child" data-content="{{  $bannerGroup['id']}}" name="input-toggle_{{ $bannerGroup['id']}}" type="checkbox" role="switch" id="switchCheckChecked_{{ $bannerGroup['id']}}">
                            </div>
                        </div>
                    </th>
                    ...
                    @foreach($fields as $index => $field)
                        @if($index == 'status' && $field['inTable'])
                            @if($bannerGroup[$index] == 1)
                                <td><div class="status_active">{{ $word['unlocked'] }}</div></td>
                            @else
                                <td><div class="status_enable">{{ $word['locked'] }}</div></td>
                            @endif
                         @elseif($index == 'image' && $field['inTable'])
                            <td><img style="width: 140px; border: 0.7px solid rgba(172, 172, 172, 0.20);" src="{{isset($bannerGroup[$index]) && $bannerGroup[$index] != '' ? 'https://i.svit-matrasiv.com.ua/images/'.$bannerGroup[$index] : '/images/common/no_images.png'}}" alt=""></td>
                         @else
                             @if($index != 'id' && $field['inTable'])
                                <td>{{ $bannerGroup[$index] }}</td>
                             @endif
                        @endif
                    @endforeach

                    <td  colspan="3">
                        {!! Form::open(['route' => ['bannerGroups.destroy', $bannerGroup->id], 'method' => 'delete']) !!}
                        <div class='btn-group'>

                            <a href="{{ route('bannerGroups.edit', [$bannerGroup->id]) }}"
    ...
    <div class="card-footer clearfix">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $bannerGroups])
        </div>
    </div>
</div>
