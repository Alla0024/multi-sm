...
<div class="form-group col-sm-6 tab-pane input-block" data-for-tab="main">
    ...
    <div class="flex-row input">
        <div class="input-group input-list-search" style="position: relative;">
            @isset($filter['option_id'])
                <input type="hidden" name="option_id" value="{{$filter['option_id']}}">
            @else
            ...
            <input
                    ...
                    placeholder="Пошук..."
                    autocomplete="off"
                    value="{{$options->firstWhere('description.option_id', $filter['option_id'])['description']['name'] ?? ''}}"
                    data-url=""
...
@endphp
@include('components.table.table_items', ['inputType' => $arrData, 'data' => $filterGroup ?? [], 'dataMultiSelect' => $options->firstWhere('description.option_id', $filter['option_id'])['optionValueGroups'] ?? '', 'name' => 'filter', 'id_name' => 'filter_id', 'tab' => 'main'])
