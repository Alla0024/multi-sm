<div class="card-body p-0">
    ...
    <div class="table-responsive">
        <table class="table" id="news-descriptions-table">
            <thead>
            <tr>
                <form class="search-form" method="GET" action="">
                    ...
                    <th class="butt-action action-item">
                        ...
                        <a href="{{ route('newsDescriptions.index') }}">{{ $word['cancel'] }}</a>
                    </th>
                </form>
            </tr>
            <tr>
            ...
            <tbody>
            @foreach($newsDescriptions as $newsDescription)
                <tr>


                    @foreach($fields as $index => $field)
                         @if($index != 'id' && $field['inTable'])
                            <td>{{ $newsDescription[$index] }}</td>
                         @endif
                    @endforeach

                    <td  colspan="3">
                        {!! Form::open(['route' => ['newsDescriptions.destroy', $newsDescription->id], 'method' => 'delete']) !!}
                        <div class='btn-group'>
                            <a href="{{ route('newsDescriptions.show', [$newsDescription->id]) }}"
                               class='btn btn-default butt-show btn-xs'>
                                <i class="bi bi-eye-fill fs-40"></i>
                            </a>
                            <a href="{{ route('newsDescriptions.edit', [$newsDescription->id]) }}"
    ...
    <div class="card-footer clearfix">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $newsDescriptions])
        </div>
    </div>
</div>
