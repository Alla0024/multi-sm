<div class="card-body p-0">
    ...
    <div class="table-responsive">
        <table class="table" id="news-to-products-table">
            <thead>
            <tr>
                <form class="search-form" method="GET" action="">
                    ...
                    <th class="butt-action action-item">
                        ...
                        <a href="{{ route('newsToProducts.index') }}">{{ $word['cancel'] }}</a>
                    </th>
                </form>
            </tr>
            <tr>
            ...
            <tbody>
            @foreach($newsToProducts as $newsToProduct)
                <tr>


                    @foreach($fields as $index => $field)
                         @if($index != 'id' && $field['inTable'])
                            <td>{{ $newsToProduct[$index] }}</td>
                         @endif
                    @endforeach

                    <td  colspan="3">
                        {!! Form::open(['route' => ['newsToProducts.destroy', $newsToProduct->id], 'method' => 'delete']) !!}
                        <div class='btn-group'>
                            <a href="{{ route('newsToProducts.show', [$newsToProduct->id]) }}"
                               class='btn btn-default butt-show btn-xs'>
                                <i class="bi bi-eye-fill fs-40"></i>
                            </a>
                            <a href="{{ route('newsToProducts.edit', [$newsToProduct->id]) }}"
    ...
    <div class="card-footer clearfix">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $newsToProducts])
        </div>
    </div>
</div>
