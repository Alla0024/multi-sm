                ...
                <form class="search-form" method="GET" action="">
                    @if(isset($fields))
                        ...
                        @foreach($fields as $index => $field)
                            @if($index != 'id' && ($field['inTable'] || $index == 'created_at'))
                                <th class="" @if($index == 'status' || $index == 'article')style="width: 132px" @endif>
                                    @if((isset($field['searchable']) && $field['searchable']) || $index == 'created_at')
                                        ...
                                        @elseif($index == 'date_value' || $index == 'created_at')
                                            <div class="">
                                                <input type="date" name="{{ $index }}" placeholder="{{ $word['search_'.$index] }}" value="{{ request($index) }}">
                                            </div>
                                        @elseif($index == 'product_id')
                                            <div class="">
                                                <input type="text" name="product_name" placeholder="{{ $word['search_'.$index] }}" value="{{ request('product_name') }}">
                                            </div>
                                        @elseif($index == 'type')
                                            <select class="" name="{{ $index }}" aria-label="{{ $word['search_'.$index] }}" aria-describedby="select-addon">
                                                <option value="all" >Всі</option>
                                                <option value="1" @if(request($index) == '1') selected @endif>Відгук</option>
                                                <option value="0" @if(request($index) == '0') selected @endif>Питання</option>
                                            </select>
                                        @else
                                            <div class="">
                                ...
                                @if($index == 'product_id')
                                    <th>
                                        ...
                                        <div class="">
                                            <input type="text" name="article" placeholder="Пошук по № моделі" value="{{ request('article') }}">
                                        </div>
                                    </th>
                                @endif
...
{{--            @dump($fields)--}}
            @foreach($productReviews as $productReview)
                <tr>
                    ...
                    @foreach($fields as $index => $field)
                        @if($index == 'status' && ($field['inTable'] || $index == 'created_at'))
                        ...
                        @elseif($index == 'product_id')
                            ...
                            <td><a style="color: #3e60d5" target="_blank" href="{{config('app.client_url').$productReview['product']['seoPath']['path']}}">{{ $productReview['product']['article'] }}</a></td>
                        @elseif($index == 'date_value' || $index == 'created_at')
                            <td>{{ date("d-m-Y",strtotime($productReview[$index])) }}</td>
                        @elseif($index == 'type')
                            <td>
                                @if($productReview[$index] == 1)
                                    <div style="padding: 4px 18px; background: #3e60d5; display: flex; width: min-content; border-radius: 4px; color: white">
                                        Відгук
                                    </div>
                                @else
                                    <div style="padding: 4px 18px; background: #ffc107; display: flex; width: min-content; border-radius: 4px; color: #333">
                                        Відгук
                                    </div>
                                @endif
                            </td>
