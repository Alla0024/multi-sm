                    ...
                    <th>
                        <div class="input-block input-toggle flex">
                            <div class="form-check form-switch">
                                <input class="form-check-input checkbox-child" data-content="{{  $province['id']}}" name="input-toggle_{{ $province['id']}}" type="checkbox" role="switch" id="switchCheckChecked_{{ $province['id']}}">
                            </div>
                        </div>
                    </th>
                    ...
                    @foreach($fields as $index => $field)
                        @if($index == 'status' && $field['inTable'])
                         ...
                         @elseif($index == 'image' && $field['inTable'])
                            <td><img style="width: 140px; border: 0.7px solid rgba(172, 172, 172, 0.20);" src="{{isset($province[$index]) && $province[$index] != '' ? 'https://i.svit-matrasiv.com.ua/images/'.$province[$index] : '/images/common/no_images.png'}}" alt=""></td>
                         @else
                             @if($index != 'id' && $field['inTable'])
                                <td>{{ $province[$index] }}</td>
                             @endif
                        @endif
                    @endforeach
